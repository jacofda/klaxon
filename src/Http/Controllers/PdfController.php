...
//use App\Classes\Accounting\Mail\SendInvoice;
use Jacofda\Klaxon\Models\{Cost, Invoice, Media, Primitive, Setting};
use Illuminate\Http\Request;
use Jacofda\Klaxon\Mail\SendInvoice;
use \PDF;
use \Storage;
    ...
    public function generate($model, $id)
    {
        ...
        if($type)
        {
            ...
            elseif($type->class == 'Cost')
            {
                ...
                $pdf = PDF::loadView('jacofda::pdf.costs.xmlTOpdf' ,compact('xml', 'title'))
...
//pdf/send/{id}
    public function sendInvoiceCortesia($id)
    {
        ...
        if($invoice->media()->pdf()->exists())
        {
            ...
            \Mail::send(new SendInvoice($name, $invoice->company));
            return 'done';
        }
        else
        {
...
//HELPERS
    ...
    {
        $class = Primitive::getClassFromDirectory($model, 'Jacofda\\Klaxon\\Models');
    ...
    private function addToDbAndSave($pdf, $filename, $model)
    {
        ...
        {
            $mediable_type =  "Jacofda\\Klaxon\\Models\\".$model->class;
    ...
    private function createInvoicePdf($invoice)
    {
        ...
        $pdf = PDF::loadView('jacofda::pdf.invoices.invoice', compact('invoice', 'company', 'title'))
                ->setOption('margin-bottom', '0mm')
