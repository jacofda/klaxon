@extends('jacofda::pdf.invoices.layout')


<div class="container">
    <div class="row">
        <div class="col-xs-4">
            <img class="img-responsive" src="{{Jacofda\Klaxon\Models\Setting::FatturaLogo()}}" style="max-width:96%;text-align: left; padding-top:60px;">
        </div>
        <div class="col-xs-4"></div>
        <div class="col-xs-4">
            <br><br>
            <p>Spett.<br><b>{{$invoice->company->rag_soc}}</b></p>
            <p>{{$invoice->company->indirizzo}}<br>
            {{$invoice->company->cap}} {{$invoice->company->citta}}, {{$invoice->company->prov}} <br>
            {{$invoice->company->nazione}}</p>
        </div>
    </div>
</div>
...
<div class="container">
    ...
    <div class="row mt-4">
        ...
        <div class="col-xs-4 p-1 pl-3 border border-bottom-0 border-right-0"><strong>P.IVA / C.F.: {{$invoice->company->piva}}</strong></div>
        <div class="col-xs-3 p-1 pl-3 border border-bottom-0"><strong>Codice SDI: {{$invoice->company->sdi}}</strong></div>
    </div>

    <div class="row">
        <div class="col-xs-5 p-1 pl-3 border border-right-0"><strong>Tipo di pagamento: {{$invoice->tipo_pagamento}}</strong></div>
        <div class="col-xs-7 p-1 pl-3 border"><strong>Riferimento: {{$invoice->riferimento}}</strong></div>
    </div>
